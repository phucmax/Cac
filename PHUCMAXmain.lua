...
local PHUCMAX_TEXT = {
    ...
    ["Base"] = "Nhà",
    ["Base / Plot"] = "Nhà / Plot",
    ["Homestead & training"] = "Nhà, máy chạy và pet",
    ["Combat"] = "Chiến đấu",
...
local okLuaLandWindow, LuaLandWindow = pcall(function()
    return LuaLandLibrary:CreateWindow({
        ...
        ToggleImage = "", -- no floating menu toggle button
        Intro = {
            Title = "PHUCMAX",
            Subtitle = "BY PHUCMAX",
            Icon = "home",
    ...
    for _, root in ipairs(roots) do
        for _, obj in ipairs(root:GetDescendants()) do
            if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                ...
                if text == "Settings" or text == "Cài đặt" then
                    local kill = obj:FindFirstAncestorWhichIsA("TextButton") or obj:FindFirstAncestorWhichIsA("ImageButton") or obj.Parent
                    if kill and kill:IsA("GuiObject") then
                        kill.Visible = false
                        kill.Active = false
                    end
                end
...
[ "performanceMode" ]=(loadedConfig.performanceMode == true ),[ "disable3D" ]=(loadedConfig.disable3D == true ),[ "antiAFK" ]=true ,[ "onTreadmill" ]= false ,[ "lastTreadmillMount" ]= 0 ,[ "laneZ" ]= -360 ,[ "swapped" ]= false ;
...
returnToSafeLine=function(e,r,eggUid,returnMode,...)
    ...
    -- Move the REAL HumanoidRootPart +80 studs before travelling home.
    -- Guard Ride only follows this position; it does not fake the altitude.
    local returnStartPos = w.Position
    local cruiseY = returnStartPos.Y + 80
    ...
    if state then state.onTreadmill=false end
...
secureEggWithGuardStrike=function(e,u,w,j,...)
    ...
    local lastRemote=0
    while os.clock()<deadline and state.alive and state.securingEgg do
        if j and farmSessionId~=j then
            break
        end
        if not state.pureTweenFarm and not state.autoFarmLoop and not state.teleporting then
        ...
        local now=os.clock()
        if e and AskFieldEggCarryRemote and now-lastRemote>=0.07 then
            lastRemote=now
            task.spawn(function()
                pcall(function()
                    if AskFieldEggCarryRemote:IsA("RemoteFunction") then
                        AskFieldEggCarryRemote:InvokeServer({["Uid"]=e})
                        AskFieldEggCarryRemote:InvokeServer(e)
                    else
                        AskFieldEggCarryRemote:FireServer({["Uid"]=e})
                        AskFieldEggCarryRemote:FireServer(e)
                    end
                end)
            end)
        end
            ...
            if AskFieldEggCarryRemote then
                pcall(function()
                    if AskFieldEggCarryRemote:IsA("RemoteFunction") then
                        ...
                        AskFieldEggCarryRemote:InvokeServer(uid)
                    else
                        AskFieldEggCarryRemote:FireServer({["Uid"] = uid})
                        AskFieldEggCarryRemote:FireServer(uid)
                    end
                end)
            end
        end
...
function HUB.AntiEggDrop.RecoverNow(uid, record)
    ...
    while HUB.AntiEggDrop.enabled and os.clock() <= deadline do
        ...
        local now = os.clock()
        if AskFieldEggCarryRemote and now - lastRemote >= 0.025 then
            lastRemote = now
            pcall(function()
                if AskFieldEggCarryRemote:IsA("RemoteFunction") then
                    AskFieldEggCarryRemote:InvokeServer({["Uid"] = uid})
...
task.spawn(function()
    while not HUB.dead do
        ...
        if not HUB.paused and autoPlantEnabled then
...
task.spawn(function()
    while not HUB.dead do
        if not HUB.paused and autoUpgradeBase then pcall(UpgradeHomesteadBase) end
        if not HUB.paused and autoUpgradeTreadmill then pcall(UpgradeTreadmillTier) end
        if not HUB.paused and autoEquipBestPets then pcall(EquipBestPets) end
        if not HUB.paused and autoClaimRewards then pcall(ClaimAllAvailableRewards) end
        if not HUB.paused and Boss.autoMastery then pcall(Boss.ClaimMastery) end
        if not HUB.paused and autoSellPets then pcall(SellSelectedPets) end
        if not HUB.paused and autoSellEggs then pcall(SellSelectedEggs) end
        task.wait(2.5)
    end
end)
...
local function phucFormatMoney(n)
    n = tonumber(n) or 0
...
local function phucNorm(s)
    return tostring(s or ""):lower():gsub("[%s_%-%[%]%(%)%.:'/\\]", "")
end

local function phucAssetsDir()
    if type(AssetsData) ~= "table" then return nil end
    return AssetsData.Directory or AssetsData.Assets or AssetsData
end

local function phucAssetInfo(category)
    local cat = tostring(category or "")
    local dir = phucAssetsDir()
    if not dir or cat == "" then return nil end
    if dir[cat] then return dir[cat] end
    local key = phucNorm(cat)
    for k, v in pairs(dir) do
        if phucNorm(k) == key then return v end
        if type(v) == "table" and (phucNorm(v.DisplayName) == key or phucNorm(v.Name) == key or phucNorm(v._id) == key) then
            return v
        end
    end
    return nil
...
local function phucImageId(value)
...
local function phucAssetIcon(category, record)
    ...
    return phucImageId(icon)
end

local function phucRecordCategory(record)
...
local function phucMutationMultiplier(record)
    local mult = 1
...
phucEggMoney = function(record)
    local cat = phucRecordCategory(record)
    local info = phucAssetInfo(cat)
    ...
    local value = raw * scale * phucMutationMultiplier(record)
    if value <= 0 then
...
local function updateEggBillboard(key, pos, record)
    ...
    local category = phucRecordCategory(record)
    ...
    bb.money.Text = "Tiền: $" .. phucFormatMoney(phucEggMoney(record)) .. " /s"
    bb.distance.Text = "Cách: " .. tostring(math.floor(dist + 0.5)) .. " studs"
    local icon = esp.showPetIcons and phucAssetIcon(category, record) or ""
    if bb.img then
        bb.img.Image = icon
        bb.img.Visible = icon ~= ""
    end
                ...
                if egg.State == "Slot" and egg.BoundsCFrame then
                    ...
                    if esp.maxDistance <= 0 or dist <= esp.maxDistance then
                        ...
                        if not esp.rareEggsOnly or isRare then
                            ...
                            updateEggBillboard(egg.Uid, pos, egg)
                        end
                    end
                end
            end
...
local EggsTab     = Window:AddTab({ Name = "Eggs", Subtitle = "Steal, hatch & plant", Icon = "home" })
local BaseTab     = Window:AddTab({ Name = "Base", Subtitle = "Homestead & training", Icon = "home" })
...
StealSub:AddToggle({
    Name = "Anti Egg Drop (0.3s Re-pick)", Default = true, Flag = "anti_egg_drop",
    Callback = safeCallback(function(v)
...
StealSub:AddParagraph({
    Title = "Chế độ cướp",
    Content = "Chỉ sử dụng Teleport để nhặt trứng và trở về."
})
StealSub:AddToggle({
...
StealSub:AddSlider({
    Name = "Glide / Travel Speed", Min = 850, Max = 1000, Default = 850, Suffix = " studs/s", Flag = "glide_speed",
    Callback = function(v) glideSpeed = tonumber(v) or 850 end
})
StealSub:AddSlider({
    Name = "Steal Delay Gap", Min = 0.02, Max = 10, Default = 0.02, Suffix = "s", Flag = "steal_gap",
...
MoveSub:AddSlider({
    Name = "WalkSpeed Value", Min = 16, Max = 1000, Default = 24, Suffix = " studs/s", Flag = "speed_val",
...
function HUB.SetEnabled(value)
    ...
    if shouldPause then
        ...
        if prev.antiAFK then pcall(function() SetAntiAFK(true) end) end
        if prev.fullbright then pcall(function() SetFullbright(true) end) end
        if prev.ride and HUB.GuardRide then pcall(function() HUB.GuardRide.Start() end) end
        if prev.fly then pcall(startFly) end
        Notify("PHUCMAX", "Đã tiếp tục hoạt động.", "Success", 3)
    end
end

do
local ControlSub = PlayerTab:AddSubTab("PHUCMAX")

ControlSub:AddToggle({
    Name = "Bật hoạt động PHUCMAX", Default = true, Flag = "phucmax_enabled",
    Callback = function(v) HUB.SetEnabled(v) end
})
ControlSub:AddParagraph({
    Title = "Cướp trứng: Teleport",
    Content = "Chỉ giữ chế độ Teleport. Tắt hoạt động sẽ tạm dừng tự động hóa; Tắt PHUCMAX sẽ thoát hẳn."
})
